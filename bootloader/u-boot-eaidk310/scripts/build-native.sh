...
for command in make gcc strings stat dtc fdtget; do
	command -v "$command" >/dev/null
done

work_root="$(mktemp -d "${TMPDIR:-/tmp}/eaidk310-uboot-native-$(date -u +%Y%m%dT%H%M%SZ)-XXXXXX")"
...
make -C "$work_source" O="$output_root/control" eaidk310-control-rk3328_defconfig
make -C "$work_source" O="$output_root/control" -j1 u-boot-dtb.bin
make -C "$work_source" O="$output_root/sdio-handoff" eaidk310-sdio-handoff-rk3328_defconfig
make -C "$work_source" O="$output_root/sdio-handoff" -j1 u-boot-dtb.bin
...
printf 'Native EAIDK310 control and SDIO-handoff builds verified under %s.\n' "$output_root"
